...
namespace WindowsGame2
{
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        private SpriteFont fontpause;//czcionka
        ...
        protected override void Update(GameTime gameTime)
        {
           ...
           if (pause.pause)
            ...
            if (!pause.pause && !gameOver&&start.start)
            {
               foreach (Asteroidy a in Asreroidy)//aktualizacja po³orzenia asteroid
                    a.Update(gameTime, new Vector2(graphics.GraphicsDevice.Viewport.Width, graphics.GraphicsDevice.Viewport.Height),win);
           ...
           if (licz < ((int)(1.5 * kolumna)))
                    szybkosc=3;//koniec sterowania szybkoœci¹ pokiemonów
         ...
         foreach (Pokemony j in Pokemon)
            {
                ...
                if (!gameOver)
                    gameOver = ((IPokemon)j).Traf(new Rectangle((int)rakieta2.Pozycja.X, (int)rakieta2.Pozycja.Y, rakieta2.Width, rakieta2.Height), gameOver);
            }
